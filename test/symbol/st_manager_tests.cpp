...
TEST_CASE("multiple scopes with symbols", "[symbol]") {
  ...
  Scope &s1 = mgr.enterScope();
  CHECK(0 == s1.getId());
  mgr.addSymbol(new Symbol("a", Types::BOOL));
  mgr.addSymbol(new Symbol("b", Types::INT));
  CHECK(mgr.lookup("a").value()->type->is(Types::BOOL));
  CHECK(!mgr.lookup("x").has_value());
  Scope &s2 = mgr.enterScope();
  mgr.addSymbol(new Symbol("a", Types::INT));
  mgr.addSymbol(new Symbol("c", Types::INT));
  CHECK(mgr.lookup("a").value()->type->is(Types::INT));
  Scope &s3 = mgr.enterScope();
  CHECK(2 == s3.getId());
  mgr.addSymbol(new Symbol("c", Types::BOOL));
  mgr.addSymbol(new Symbol("d", Types::INT));
  CHECK(mgr.lookup("b").value()->type->is(Types::INT));
  CHECK(mgr.lookup("c").value()->type->is(Types::BOOL));
  mgr.exitScope();
  Scope &s4 = mgr.enterScope();
  CHECK(s4.getParent() == &s2);
  mgr.addSymbol(new Symbol("a", Types::INT));
  mgr.addSymbol(new Symbol("avar", Types::BOOL));
  CHECK(mgr.lookup("c").value()->type->is(Types::INT));
